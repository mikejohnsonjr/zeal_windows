...
uint256 hashGenesisBlock("0xebd032b618c0cd30d93415078893abfc9a7d0275883a3d5f6db81b08c20dd88e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Zeal: starting difficulty is 1 / 2^12
...
{
    // Zeal: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Zeal: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Zeal: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Zeal: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Zeal: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Zeal: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4ebb28761c93d288a8ab82931290e56330f95a06f7eb24435de28ef21aa71261");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "02/11/2020 - Andrew Yang drops out of presidential race";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a17966b9927e377afbab864b0cdbcbc966ddde81fc6a4ac85477fa3fac559d36d981f2b93a61c4074bcebc95a694c833b7558aab6542ca68e04bc202449e6435") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1581473874;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2089188660 ;

        if (fTestNet)
        {
            block.nTime    = 1581473839;
            block.nNonce   = 386031812;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x0957f2aca8299067f22038a0902f478df399cf94e3fcb1fa1f34c1699958457a"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc2, 0xb4, 0xdc }; // Zeal: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ZealMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ZealMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ZealMiner(CWallet *pwallet)
{
    printf("ZealMiner started\n");
    ...
    try { loop {
        ...
        printf("Running ZealMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ZealMiner, pwallet));
}
